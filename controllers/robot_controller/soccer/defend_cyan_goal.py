# =============================================================================
# defend_cyan_goal.py
# Controller for a robot DEFENDING THE CYAN GOAL (left side of field)
#
# Field orientation (top-down view):
#   CYAN goal  <---  [-1.1]  ...field...  [+1.1]  ---> MAGENTA goal
#
# This robot:
#   - Starts at approximately (-0.63, 0), facing +x (toward magenta/opponent)
#   - Own goal:      CYAN     at x ≈ -1.15  (BEHIND the robot)
#   - Opponent goal: MAGENTA  at x ≈ +1.15  (AHEAD of the robot)
#   - Camera faces forward (+x) → sees opponent goal and ball when facing right
#   - ds_back sensor points toward own cyan goal
#
# Strategy:
...
# Role assignments for THIS controller (defending cyan)
def is_own_goal(r, g, b):
    """Own goal is CYAN — seeing this means we're facing the wrong way."""
    return is_cyan(r, g, b)

def is_opponent_goal(r, g, b):
    """Opponent goal is MAGENTA — camera sees this when facing correctly."""
...
while robot.step(TIME_STEP) != -1:
    ...
    if state == 'ATTACK':
        ...
        # ── RETREAT: back up toward cyan goal (which is behind this robot) ──
