...
# -----------------------------
# Phased SCAN (bounded sweep)
# NOTE: "steps" are controller iterations, NOT degrees.
# Tune steps_for_60 until it looks like ~60° in your sim.
# -----------------------------
steps_for_60 = 8
...
while robot.step(timestep) != -1:
    ...
    # =============================
    if state == SCAN:
        # 3-phase sweep: +60, -120, +60 (bounded; avoids endless 360)
        if scan_phase == 1:
            ...
            if scan_steps >= steps_for_60:
                scan_phase = 1
                scan_steps = 0
                # After one sweep, repeat sweep (bounded pattern, not a continuous 360)
    ...
    # =============================
        ...
        if green_pos == "LEFT":
            ...
            # Lost gate -> go back to bounded SCAN sweep (not infinite spin in TRACK)
            state = SCAN
            scan_phase = 1
            scan_steps = 0
